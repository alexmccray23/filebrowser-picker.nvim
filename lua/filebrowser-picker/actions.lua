    ...
    vim.ui.select({ "No", "Yes" }, {
      ...
      for _, itm in ipairs(selected_items) do
        ...
        if vim.fn.filereadable(to) == 1 or vim.fn.isdirectory(to) == 1 then
          ...
          -- Use Snacks rename if available, fallback to libuv
          local ok, err = pcall(function()
            local Snacks = require "snacks"
            if Snacks.rename and Snacks.rename.rename_file then
              Snacks.rename.rename_file { from = from, to = to }
            else
              local success, move_err = M._move_file(from, to)
              if not success then
                error("Move failed: " .. (move_err or "unknown error"))
              end
            end
          end)
    ...
    vim.ui.input({
      ...
      local ok, err = pcall(function()
        ...
        if Snacks.rename and Snacks.rename.rename_file then
          ...
          local success, move_err = M._move_file(single_item.file, dest_path)
          if not success then
            error("Move failed: " .. (move_err or "unknown error"))
          end
        end
...
---Cross-platform file copy using libuv
---@param src string Source path
---@param dest string Destination path
function M._copy_path(src, dest)
  local uv = vim.uv or vim.loop
  ...
  if src_stat.type == "file" then
    -- Copy file using libuv
    local success, err = uv.fs_copyfile(src, dest)
    if not success then
      error("Failed to copy file: " .. (err or "unknown error"))
    end
  elseif src_stat.type == "directory" then
    -- Recursively copy directory
    M._copy_directory(src, dest)
  else
    error("Unsupported file type: " .. src_stat.type)
  end
end

---Recursively copy directory using libuv
---@param src_dir string Source directory
---@param dest_dir string Destination directory
function M._copy_directory(src_dir, dest_dir)
  local uv = vim.uv or vim.loop

  -- Create destination directory
  local success, err = uv.fs_mkdir(dest_dir, tonumber("755", 8))
  if not success and err and not err:match "EEXIST" then
    error("Failed to create directory: " .. dest_dir .. " (" .. err .. ")")
  end

  -- Copy directory contents
  local handle = uv.fs_scandir(src_dir)
  if not handle then
    error("Failed to read directory: " .. src_dir)
  end
  ...
  while true do
    ...
    local src_path = vim.fs.joinpath(src_dir, name)
    local dest_path = vim.fs.joinpath(dest_dir, name)

    if type == "file" then
      uv.fs_copyfile(src_path, dest_path)
    elseif type == "directory" then
      M._copy_directory(src_path, dest_path)
    end
  end
end

---Cross-platform file move using libuv
---@param from string Source path
---@param to string Destination path
---@return boolean success
---@return string? error
function M._move_file(from, to)
  local uv = vim.uv or vim.loop
  local success, err = uv.fs_rename(from, to)
  return success, err
end

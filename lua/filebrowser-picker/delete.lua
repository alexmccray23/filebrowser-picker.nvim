...
local function get_confirmation_message(selected_items, opts)
  ...
  if #non_empty_dirs > 0 then
    local dir_warning = #non_empty_dirs == 1 and "'" .. non_empty_dirs[1] .. "' contains files" or #non_empty_dirs .. " directories contain files"
    ...
    if opts.use_trash and find_trash_cmd() then
      ...
      return base_message .. "\n" .. dir_warning .. " - choose option:", { "Cancel", "Delete (files only)", "Delete recursively" }
    end
  end

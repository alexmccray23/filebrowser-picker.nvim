...
local function install_format_patch(actions, util_mod, stat_mod, git_mod)
	...
	actions.format_item = function(item, opts)
		...
		if item.dir then
			icon, icon_hl = util_mod.icon(
				item.text,
				"directory",
				{ fallback = { file = (opts.icons and opts.icons.folder_closed) or "󰉋" } }
			)
			text_hl = "Directory"
		elseif item.type == "link" then
			...
			icon, icon_hl =
				util_mod.icon(item.text, "file", { fallback = { file = (opts.icons and opts.icons.file) or "󰈔" } })
		end
		...
		if opts.git_status and git_mod and git_mod.get_status_sync then
			...
			if st then
				git_icon = (
					opts.icons
					and opts.icons.git
					and (st == "staged" and opts.icons.git.staged or opts.icons.git[st])
				) or git_icon

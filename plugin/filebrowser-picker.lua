...
vim.api.nvim_create_user_command("FileBrowserPickerProfile", function(opts)
  local args = {}
  if opts.args and opts.args ~= "" then
    args.path = vim.fn.expand(opts.args)
  end
  require("filebrowser-picker.bench").profile_scanners(args)
end, {
  nargs = "?",
  complete = "dir",
  desc = "Profile file browser scanner performance",
})
